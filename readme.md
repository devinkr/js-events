...
finishes - but not in JavaScript.

JavaScript is an asynchronous programming language, which simply means things
won't necessarily happen one after the other.

Instead, what would likely happen is `doWork` would get called and start it's
process, then `getPaid` would get called and then sometime later, `doWork` would
finish running.
...
`getPaid` is a separate function:

```js
function getPaid() {
  const message = "I got mooooooooney 💰🤑🕺";
...
Note that we're passing a _reference_ to `getPaid` in to `doWork`.
...
Visit this
[repository](https://git.generalassemb.ly/seir-323/event-listener-demo)
and follow the instructions.
